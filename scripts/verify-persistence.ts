...
import { createPersistenceTracker, type PersistenceState } from '../src/utils/persistence';
...
async function main() {
  ...
  assert.equal(state().error, '生词尚未保存：still full', 'Keep and update only the remaining failure');
  failWords = false;
  await tracker.retryAll();
  ...
  saveGate.resolve();
  await firstRetry;
  assert.equal(state().error, '书架尚未保存：disk full', 'A failure arriving during retry must remain visible');
  await tracker.retryAll();
  ...
  oldRetry.resolve();
  await retrying;
  assert.equal(state().error, '生词尚未保存：new failure');
  await tracker.retryAll();
  assert.equal(state().error, null);
